...
class GruenbeckCollector(object):
    ...
    def collect(self):          
        ...
        next_regeneration = GaugeMetricFamily("gruenbeck_next_regeneration", "Timestamp for next regeneration")
        try:
        ...
        raw_water = GaugeMetricFamily("gruenbeck_raw_water", "Raw water hardness")
        ...
        # softWater
        soft_water = GaugeMetricFamily("gruenbeck_soft_water", "Soft water hardness")
        ...
        mode = GaugeMetricFamily("gruenbeck_mode", "Current mode (1=Eco, 2=Comfort, 3=Power)")
        ...
        # Has error
        # hasError
        hasError = GaugeMetricFamily("gruenbeck_has_error", "Water softener has error")
        ...
        ### Stream
        # mcountwater1
        water_usage = GaugeMetricFamily("gruenbeck_water_usage", "Soft water usage of all time in liters", labels=['tank'])
        for idx, x in enumerate(self.__gbApi.waterUsages):
            water_usage.add_metric([str(idx+1)], x)
        yield water_usage

        # mcountreg
        reg_counter = GaugeMetricFamily("gruenbeck_reg_counter", "Regeneration counter")
        reg_counter.add_metric([], self.__gbApi.regenerationCounter)
        yield reg_counter

        # mflow1
        water_flow = GaugeMetricFamily("gruenbeck_water_flow", "Actual water flow in m3/h", labels=['tank'])
        for idx, x in enumerate(self.__gbApi.waterFlows):
            water_flow.add_metric([str(idx+1)], x)
        yield water_flow

        # mrescapa1
        remaining_cap_m3 = GaugeMetricFamily("gruenbeck_remaining_cap_m3", "Remaining capacity in m3", labels=['tank'])
        for idx, x in enumerate(self.__gbApi.remainingCapacitiesM3):
            remaining_cap_m3.add_metric([str(idx+1)], x)
        yield remaining_cap_m3

        # residcap1
        remaining_cap_percent = GaugeMetricFamily("gruenbeck_remaining_cap_percent", "Remaining capacity in %", labels=['tank'])
        ...
        # msaltrange
        salt_range = GaugeMetricFamily("gruenbeck_salt_range", "Salt range in days")
        salt_range.add_metric([], self.__gbApi.saltRange)
        yield salt_range

        # msaltusage
        salt_usage = GaugeMetricFamily("gruenbeck_salt_usage", "Salt usage in kg")
        salt_usage.add_metric([], self.__gbApi.saltUsage)
        yield salt_usage

        # Maintenance left days
        # mmaint
        maint_left = GaugeMetricFamily("gruenbeck_maint_left", "Left days until next maintenance")
        maint_left.add_metric([], self.__gbApi.maintenanceLeftDays)
        yield maint_left

        # Regeneration status
        # regstatus
        reg_status = GaugeMetricFamily("gruenbeck_reg_status", "Regeneration status")
        reg_status.add_metric([], self.__gbApi.regenerationStatus)
        yield reg_status
